...
export class Client extends EventEmitter {
    ...
    constructor(host: string, port: number, user: string, pass: string, callback: (res: any) => void) {
        super();
        this.host_ = host;
        this.port_ = port;
        this.user_ = user;
    ...
    private issue_(req: any, body: string | null, callback: (res: any) => void): void {
        if (!this.socket_) {
        ...
        const text = req.method + ' ' + req.path + ' HTTP/1.1\n' + allHeaders + '\n';
        if (body) {
            text += body;
        }
        this.responseWaiters_.push({ callback });
